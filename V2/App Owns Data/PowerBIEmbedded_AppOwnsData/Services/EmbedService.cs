...
namespace PowerBIEmbedded_AppOwnsData.Services
{
    public class EmbedService : IEmbedService
    {
        ...
        public async Task<bool> EmbedReport(string username, string roles, string reporte, string grupo)
        {
            // Get token credentials for user
            var getCredentialsResult = await GetTokenCredentials();
            if (!getCredentialsResult)
            ...
            try
            {
                ...
                {
                    // Get a list of reports.
                    var reports = await client.Reports.GetReportsInGroupAsync(grupo);
                    ...
                    {
                        report = reports.Value.FirstOrDefault(r => r.Id.Equals(reporte, StringComparison.InvariantCultureIgnoreCase));
                    }
                    ...
                    var datasets = await client.Datasets.GetDatasetByIdInGroupAsync(grupo, report.DatasetId);
                    m_embedConfig.IsEffectiveIdentityRequired = datasets.IsEffectiveIdentityRequired;
                    m_embedConfig.IsEffectiveIdentityRolesRequired = datasets.IsEffectiveIdentityRolesRequired;
                    GenerateTokenRequest generateTokenRequestParameters;
                    // This is how you create embed token with effective identities
