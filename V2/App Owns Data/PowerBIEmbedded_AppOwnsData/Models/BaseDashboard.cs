...
namespace PowerBIEmbedded_AppOwnsData.Models
{
    public class BaseDashboard
    {
        ...
        public List<PowerBiDash> dashboards;

        public BaseDashboard()
        {
            dashboards = new List<PowerBiDash>();
            dashboards.Add(new PowerBiDash(titulo: "Nombre", id:1, id2:1, url: "https://app.powerbi.com/view?r=eyJrIjoiN2RiNDAxYWQtZjIyZi00ZmQ5LWE0M2UtOTBkMGQ3NWEyNGM5IiwidCI6IjhmYmFhNWJmLTJlY2MtNGRjOC1iNTZiLThmOTJlMzA3ZjA3NiIsImMiOjR9&pageName=ReportSectiond3ed353228b160739c25",comentario:"Prueba"));
        }

        public PowerBiDash buscarDashboard(int id=1, int id2= 2)
        {
            PowerBiDash salida;
            try
            {
                salida = this.dashboards.Where(x => x.id == id && x.id2 == id2).First();
            }
            catch (Exception)
            {
                salida = null;
